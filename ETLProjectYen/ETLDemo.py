...
if (BOCResponse.status_code == 200):
    ...
    for row in BOCRaw['observations']:
        ...
        BOCRates.append(decimal.Decimal(row['FXJPYCAD']['v']))
    ...
    expenses = petl.select(expenses,lambda rec: rec.JPY != None)
    print(expenses)

    # add CDN column
    expenses = petl.addfield(expenses,'CAD', lambda rec: decimal.Decimal(rec.JPY) * rec.rate)
    print(expenses)
    ...
    try:
            petl.io.todb (expenses,conn,'ExpensesJPY')
    except Exception as e:
